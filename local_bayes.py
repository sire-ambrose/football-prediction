...
def dist(team1, team2, data):
    ...
    mylabels = [team1, "Draw", team2]
    circle= pd.DataFrame([(team1, team1_win),( "Draw", draw), (team2,team2_win)], columns=['Teams', 'Odds of Winning'])
    ...
    dt= pd.DataFrame(disp*100)
    print('overall...')
    over= pd.concat([dt, circle, bar1, bar2], axis=1)
    over['Goals']=over['Goals'].astype('str')
    over.to_csv('overall.csv', index=False)
